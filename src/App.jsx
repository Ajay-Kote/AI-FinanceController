    ...
    <Modal open={open} onClose={onClose} title="Make Payment" size="sm">
      <div className="space-y-4">
        <div className="rounded-lg bg-brand-50 p-3 text-xs text-brand-700 dark:bg-brand-950/40 dark:text-brand-300">
          ...
          <p className="mt-1">Payments are processed in INR via Razorpay test mode. After successful payment, a transaction is automatically created in your dashboard.</p>
        </div>
        ...
        <div>
          ...
          {amount && parseFloat(amount) > 0 && (
            <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
              Approx {formatCurrency(parseFloat(amount))} → ₹{Math.round(parseFloat(amount) * 83).toLocaleString('en-IN')} INR
            </p>
          )}
        </div>
